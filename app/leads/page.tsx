...
function PriorityBadge({ priority }: { priority: string }) {
  const map: Record<string, string> = {
    High: "text-rose-600 bg-rose-50", Medium: "text-amber-600 bg-amber-50", Low: "text-slate-500 bg-slate-100",
  };
  return <span className={`text-xs font-semibold px-2 py-0.5 rounded ${map[priority] || map.Low}`}>{priority}</span>;
}

function ActivityItem({ act }: { act: Activity }) {
  const icon = ACTIVITY_ICONS[act.type] || "•";
  const timeStr = new Date(act.createdAt).toLocaleString("en-IN", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" });
  return (
    <div className="flex gap-3 py-3 border-b border-slate-100 last:border-0">
      <div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-base flex-shrink-0">{icon}</div>
      <div className="flex-1 min-w-0">
        <p className="text-sm text-slate-800 leading-snug">{act.note}</p>
        {act.fromStage && act.toStage && <p className="text-xs text-slate-400 mt-0.5">{act.fromStage} → {act.toStage}</p>}
  ...
  async function submit() {
    ...
    try {
      ...
      if (type === "stage_change" && newStage !== lead.stage) {
        payload.stage = newStage; payload.note = note;
      } else {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
          ...
          <div className="grid grid-cols-4 gap-2">
            {(["note","call","whatsapp","email","visit_scheduled","visit_done","document","stage_change"] as ActivityType[]).map(t => (
              <button key={t} onClick={() => setType(t)}
                className={`flex flex-col items-center gap-1 py-2 px-1 rounded-xl border text-xs font-medium transition-all ${type === t ? "border-indigo-500 bg-indigo-50 text-indigo-700" : "border-slate-200 text-slate-500 hover:border-slate-300"}`}
              >
        ...
        <div className="mb-5">
          ...
          <textarea value={note} onChange={e => setNote(e.target.value)} rows={3} placeholder="What happened? Any context..."
        ...
        <div className="flex gap-2">
          ...
          <button onClick={submit} disabled={loading || !note.trim()} className="flex-1 py-2.5 rounded-xl bg-indigo-600 text-white text-sm font-medium hover:bg-indigo-700 disabled:opacity-50">
...
function LeadDetailPanel({ lead, onClose, onUpdate }: { lead: Lead; onClose: () => void; onUpdate: (updated: Lead) => void }) {
  const [showActivityModal, setShowActivityModal] = useState(false);

  async function changeStage(stage: LeadStage) {
    if (stage === lead.stage) return;
    const res = await fetch(`/api/leads/${lead._id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ stage, performedBy: "Admin" }) });
    const data = await res.json();
    onUpdate(data.lead);
  }
        ...
        <div className="flex flex-wrap gap-1.5">
          {STAGES.map(s => (
            <button key={s} onClick={() => changeStage(s)}
              className={`text-xs px-3 py-1.5 rounded-lg border font-medium transition-all ${lead.stage === s ? `${STAGE_CONFIG[s].bg} ${STAGE_CONFIG[s].color} border-current` : "border-slate-200 text-slate-500 hover:border-slate-300 hover:bg-slate-50"}`}
            >{s}</button>
          ))}
        </div>
      </div>
      <div className="px-6 py-4 border-b border-slate-100 grid grid-cols-2 gap-3 text-sm">
        {([["Source", lead.source], ["Priority", <PriorityBadge key="p" priority={lead.priority} />], ["Assigned To", lead.assignedTo], ["Zone", lead.zone || "—"], ["Property", lead.propertyType || "—"], ["Budget", lead.budget || "—"], ["Locality", lead.preferredLocality || "—"], ["Possession", lead.possession || "—"]] as [string, React.ReactNode][]).map(([label, value]) => (
          <div key={label}><p className="text-xs text-slate-400">{label}</p><p className="font-medium text-slate-800 mt-0.5">{value}</p></div>
        ))}
        {lead.email && <div className="col-span-2"><p className="text-xs text-slate-400">Email</p><p className="font-medium text-slate-800 mt-0.5">{lead.email}</p></div>}
        {lead.notes && <div className="col-span-2"><p className="text-xs text-slate-400">Notes</p><p className="text-slate-700 mt-0.5 text-sm leading-relaxed">{lead.notes}</p></div>}
      </div>
      <div className="flex-1 overflow-y-auto px-6 py-4">
        <div className="flex items-center justify-between mb-3">
          <p className="text-xs font-medium text-slate-400 uppercase tracking-wide">Activity Log ({lead.activities.length})</p>
          <button onClick={() => setShowActivityModal(true)} className="text-xs font-medium text-indigo-600 hover:text-indigo-800 bg-indigo-50 px-3 py-1.5 rounded-lg">+ Log Activity</button>
        </div>
        {lead.activities.length === 0 ? (
          <p className="text-sm text-slate-400 text-center py-8">No activity yet.</p>
        ) : (
          [...lead.activities].reverse().map(act => <ActivityItem key={act._id || act.createdAt} act={act} />)
        )}
      </div>
      {showActivityModal && (
        <AddActivityModal lead={lead} onClose={() => setShowActivityModal(false)} onSaved={updated => { onUpdate(updated); setShowActivityModal(false); }} />
      )}
    </motion.div>
  );
}
...
function LeadCard({ lead, onClick }: { lead: Lead; onClick: () => void }) {
  ...
  return (
    <motion.div layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} whileHover={{ y: -2 }} onClick={onClick}
    ...
    >
      <div className="flex items-start justify-between gap-2 mb-2">
        <div className="min-w-0">
          <p className="font-semibold text-slate-900 text-sm truncate group-hover:text-indigo-600 transition-colors">{lead.name}</p>
...
export default function LeadsPage() {
  ...
  function onLeadUpdated(updated: Lead) { setLeads(prev => prev.map(l => l._id === updated._id ? updated : l)); setSelectedLead(updated); }

  const stageGroups = STAGES.reduce((acc, s) => { acc[s] = leads.filter(l => l.stage === s); return acc; }, {} as Record<LeadStage, Lead[]>);
    ...
    <div className="min-h-screen bg-slate-50">
        ...
        <div className="flex bg-slate-100 rounded-xl p-1 gap-1">
          {(["pipeline", "list"] as const).map(v => (
            <button key={v} onClick={() => setViewMode(v)}
              className={`px-3 py-1.5 rounded-lg text-sm font-medium capitalize transition-all ${viewMode === v ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
            >{v === "pipeline" ? "⊞ Pipeline" : "☰ List"}</button>
          ))}
        </div>
      ...
      <div className="px-6 py-3 bg-white border-b border-slate-100 flex items-center gap-2 overflow-x-auto">
        <button onClick={() => setStageFilter("All")}
          className={`px-4 py-1.5 rounded-lg text-sm font-medium whitespace-nowrap transition-all ${stageFilter === "All" ? "bg-slate-900 text-white" : "text-slate-500 hover:bg-slate-100"}`}
        >All ({leads.length})</button>
        {STAGES.map(s => (
          <button key={s} onClick={() => setStageFilter(stageFilter === s ? "All" : s)}
            className={`px-4 py-1.5 rounded-lg text-sm font-medium whitespace-nowrap transition-all ${stageFilter === s ? `${STAGE_CONFIG[s].bg} ${STAGE_CONFIG[s].color} border ${STAGE_CONFIG[s].bg}` : "text-slate-500 hover:bg-slate-100"}`}
          >{s} ({stageGroups[s]?.length || 0})</button>
        ))}
      </div>
        ...
        {loading ? (
              ...
              return (
                <div key={stage} className="flex-shrink-0 w-72">
                  ...
                  <div className="space-y-2">
                    <AnimatePresence>
                      {stageLeads.map(lead => <LeadCard key={lead._id} lead={lead} onClick={() => setSelectedLead(lead)} />)}
                    </AnimatePresence>
                    {stageLeads.length === 0 && (
          ...
          <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody>
                {leads.map(lead => (
                  <tr key={lead._id} onClick={() => setSelectedLead(lead)} className="border-b border-slate-50 hover:bg-slate-50 cursor-pointer transition-colors">
                    ...
                    <td className="px-4 py-3 text-slate-400 text-xs">{new Date(lead.createdAt).toLocaleDateString("en-IN")}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            {leads.length === 0 && <div className="py-16 text-center text-slate-400">No leads found.</div>}
          </div>
        )}
      </div>
      ...
      <AnimatePresence>
        {selectedLead && (
          <LeadDetailPanel lead={selectedLead} onClose={() => setSelectedLead(null)} onUpdate={onLeadUpdated} />
        )}
      </AnimatePresence>
    </div>
  );
